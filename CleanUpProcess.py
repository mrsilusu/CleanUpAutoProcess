            ...
            for page_num, page in enumerate(pdf.pages):
                ...
                for table_num, table in enumerate(tables):
                    ...
                    try:
                        ...
                        st.warning(f"Erro na página {page_num+1}, tabela {table_num+1}: {e}")
                        continue
...
# ==============================
# Extração de Dados do Excel em Memória
# ==============================
    ...
    for df in dataframes:
        ...
        for row_idx in range(len(df_str)):
            for col_idx in range(len(df_str.columns)):
                ...
                # Busca flexível por variações do cabeçalho
                if any(term in celula for term in ["fim da fibra", "fim fibra", "end fiber"]) and "km" in celula:
                    st.sidebar.success(f"✅ Encontrado no Excel: linha {row_idx+1}, coluna {col_idx+1}")
                    
                    # Pegar valor da LINHA ABAIXO no Excel
                    if row_idx + 1 < len(df_str):
                        ...
                        if numero_limpo:
                            try:
                                ...
                                st.sidebar.info(f"📏 Distância extraída do Excel: {distancia} km")
                                return distancia
                            except ValueError:
                                continue
    
    st.sidebar.warning("⚠️ Não foi possível encontrar o Fim da Fibra automaticamente")
    return None
        ...
        for row_idx in range(len(df_str)):
            for col_idx in range(len(df_str.columns)):
                ...
                if "perda total" in celula and "db" in celula:
                    # Valor está na mesma linha ou linha abaixo
                    if row_idx + 1 < len(df_str):
                        ...
                        if perda_limpa:
                            try:
                                ...
                                st.sidebar.info(f"📊 Perda total extraída do Excel: {perda_total} dB")
                            except ValueError:
                                continue
...
def processar_excel_memoria(excel_file, uploaded_file, quadrimestre, distancia_troco_km, perda_maxima_dB):
    ...
    try:
        ...
        return {
            ...
            "Distância Testada (km)": distancia_fibra,  # ← Do Excel
    ...
    if file_prev and file_curr:
        ...
        if resultado_prev and resultado_curr:
            ...
            with col1:
                ...
                st.metric("Distância Testada", f"{resultado_prev['Distância Testada (km)']} km")
                st.metric("Perda Total", f"{resultado_prev['Perda Total (dB)']} dB")
            ...
            with col2:
                ...
                st.metric("Distância Testada", f"{resultado_curr['Distância Testada (km)']} km")
                st.metric("Perda Total", f"{resultado_curr['Perda Total (dB)']} dB")
                st.metric("Status", resultado_curr["Status"])
                st.metric("Total de Tabelas", resultado_curr["Total_Tabelas"])
            
            # Informações Adicionais
            with st.expander("📋 Informações Técnicas do Processamento"):
                st.write(f"**Arquivo Anterior:** {file_prev.name}")
                st.write(f"**Arquivo Atual:** {file_curr.name}")
                st.write(f"**Data/Hora do Processamento:** {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
                st.write(f"**Perda Máxima Calculada:** {perda_maxima:.2f} dB")
